...
exports.parseRawToNormalized = parseRawToNormalized;
...
function deriveCanonicalUrl(rawUrl, html) {
...
function deriveBoeUid(url, html) {
...
function extractAmountWithLabels(html) {
    return (extractRegex(html, /Valor\s+subasta[\s\S]{0,120}?([\d\.\,]+)/i) ||
...
function normalizeEstado(estado) {
    ...
    if (low.includes("cancel") || low.includes("suspend"))
        return types_1.EstadoNormalizado.CANCELLED;
    if (low.includes("finaliz") || low.includes("conclu"))
        return types_1.EstadoNormalizado.CLOSED;
    if (low.includes("prox") || low.includes("apertura"))
        return types_1.EstadoNormalizado.UPCOMING;
    if (low.includes("celebr") || low.includes("activa") || low.includes("puja"))
        return types_1.EstadoNormalizado.ACTIVE;
    return types_1.EstadoNormalizado.UNKNOWN;
}
function parseRawToNormalized(raw) {
